...
terminal_in_special() {
    local addr=$(get_terminal_address)
    if [ -n "$addr" ]; then
        hyprctl clients -j | jq -e --arg ADDR "$addr" 'any(.[]; .address == $ADDR and .workspace.name == "special:SP")' >/dev/null 2>&1
    else
        return 1
    fi
}
...
spawn_terminal() {
    ...
    hyprctl dispatch exec "[float; size $width $height; workspace special:SP silent] $TERMINAL_CMD"
...
if terminal_exists; then
    ...
    if terminal_in_special; then
        debug_echo "Bringing terminal from SP with slide down animation"
        ...
        debug_echo "Hiding terminal to SP with slide up animation"
        ...
        if [ -n "$geometry" ]; then
            ...
            debug_echo "Could not get window geometry, moving to SP without animation"
            hyprctl dispatch pin "address:$TERMINAL_ADDR"
            hyprctl dispatch movetoworkspacesilent "$SPECIAL_WS,address:$TERMINAL_ADDR"
        fi
    fi
